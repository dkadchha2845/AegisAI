...
### Trainable Models
1. **MuRIL Stage Classifier** (`ml/training/train.py`)
2. **Digital Twin Transition Matrix** (`ml/corpus/build_dataset.py`)

### Dataset Generation (`ml/corpus/generate_calls.py` -> `ml/corpus/paraphrase.py`)
...
### Preprocessing & Splitting (`ml/corpus/build_dataset.py`)
...
### Model Fine-Tuning (`ml/training/train.py`)
...
### The Digital Twin Fitting (`ml/corpus/build_dataset.py :: fit_transitions`)
...
ml/
 ├── artifacts/
 │   ├── stage-classifier/            # Generated by ml/training/train.py
 ...
 │   │   └── ...
 │   ├── backend_comparison.json      # Generated by ml/evaluation/eval_backends.py
 │
 ├── data/
 │   ├── processed/
...
### 9. `ml/corpus/build_dataset.py` & `ml/training/train.py` (Data Pipeline)
